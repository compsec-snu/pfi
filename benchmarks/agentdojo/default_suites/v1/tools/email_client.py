...
from default_suites.v1.tools.types import CalendarEvent, CloudDriveFileID, Email, EmailID, EmailStatus
...
class Inbox(BaseModel):
    ...
    def _create_emails(self) -> Self:
        ...
        if len(all_ids) != len(set(all_ids)):
            ...
            raise ValueError(f"Email IDs must be unique.")
    ...
    def search_emails(self, query: str, sender: str | None = None) -> list[Email]:
    ...
    def delete_email(self, email_id: EmailID) -> Email:
        if email_id not in self.emails:
            raise ValueError(f"Email with given ID not found.")
    ...
    def find_contacts_by_name(self, name: str) -> list[EmailContact]:
        ...
        if len(matches) == 0:
            raise ValueError(f"Contact with given name not found.")
        return matches

    def find_contacts_by_email(self, email: str) -> list[EmailContact]:
        matches = []
        for contact in self.contact_list:
            if email.lower() in contact.email.lower():
                matches.append(contact)
        if not matches:
            raise ValueError(f"Contact with given email not found.")
        return matches
...
env = EmailEnvironment(inbox=inbox)

@pfi_tool
def get_unread_emails() -> PFIPluginResult:
    ...
    res = [PFIData(data=email, attr="email:"+email.sender) for email in env.inbox.get_unread()]
...
def send_email(
    ...
    :param attachments: The list of attachments. If `null`, no attachments are included.
    If the attachment has as "type" "file", it is expected to have a field "file_id", with the ID of the file in the cloud drive. If the attachment has as "type" "event", it is expected to be a calendar event in the field "event_details".
        ...
        for attachment in attachments:
            if not isinstance(attachment, dict):
                try:
                    ...
                    raise ValueError(f"Attachments must be dictionaries: {type(attachment)}")
    ...
    res = env.inbox.send_email(recipients, subject, body, parsed_attachments, cc, bcc)
...
def search_emails(
    ...
    """
    matches = env.inbox.search_emails(query, sender)
    if len(matches) == 0:
        raise ValueError("No emails found. Try with a different query.")
    res = [PFIData(data=email, attr="email:"+email.sender) for email in matches]
    return PFIDataList(data=res)

@pfi_tool
def delete_email(email_id: str) -> PFIPluginResult:
    """Deletes the email with the given `email_id` from the inbox.

    :param email_id: The id of the email to delete.
    """
    env.inbox.delete_email(email_id)
    return PFIData(data=f"Email with id {email_id} deleted successfully.", attr="tool:email_client")

@pfi_tool
def get_sent_emails() -> PFIPluginResult:
    """Returns all the sent emails in the inbox. Each email has a recipient, a subject, and a body."""
    return PFIData(data=env.inbox.sent, attr="user")

@pfi_tool
def get_received_emails() -> PFIPluginResult:
    """Returns all the received emails in the inbox. Each email has a sender, a subject, and a body."""
    res = [PFIData(data=email, attr="email:"+email.sender) for email in env.inbox.received]
    return PFIDataList(data=res)

@pfi_tool
def get_draft_emails() -> PFIPluginResult:
    """Returns all the draft emails in the inbox. Each email has a recipient, a subject, and a body."""
    return PFIData(data=env.inbox.drafts, attr="user")

@pfi_tool
def search_contacts_by_name(query: str) -> PFIPluginResult:
    ...
    """
    return PFIData(env.inbox.find_contacts_by_name(query), attr="user")

@pfi_tool
def search_contacts_by_email(query: str) -> PFIPluginResult:
    ...
    """
    return PFIData(env.inbox.find_contacts_by_email(query), attr="user")
