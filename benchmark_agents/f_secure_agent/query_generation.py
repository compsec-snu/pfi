...
from .utils import print_debug, get_tool_messages
...
def validate_ast_format(query):
    ...
    if isinstance(query, dict):
        ...
        if query == []:
            ...
            query.append("string")
        if len(query) != 1:
            error_msg = "Error: List format should contain one format (bool, int, float, email, url, phone, date, datetime, time, and string) or a nested type. "
            return None, error_msg
        elem_format = query[0]
...
def generate_query(llm, state, tool_calls, all_tools):
    query_generator = llm
    ...
    while True:
        ...
        if validated_format is None:
            ...
            print(f"Query: {validated_format}")
