...
Use conditional_prompt with data proxy(s) when comparing the raw data of the proxy.
...
(Possible data types: bool, int, float, email, url, phone, date (YYYY-MM-DD), datetime (YYYY-MM-DD HH:MM), time (HH:MM), prompt, and string)
...
--- WARNING ---
Query is populated by {tool_call}'s result and {untrusted_agent_tools}.
Specify the most descriptive and specific information to extract.
The leaf value should be one of the data types: bool, int, float, email, url, phone, date, datetime, time, prompt, and string.
List contains only one data type or a nested type.
Use `prompt` to extract actions, instructions, or tasks.
Return only the query in JSON format without any extra text.
----------------
"""

CONDITIONAL_PROMPT_MESSAGE="""
Given the data: {data}, evaluate condition: {code}
and choose the appropriate prompt from the list: {prompts}.
Return the selected prompt. 
WARNING: Do not add any extra text or explanations and only return the selected prompt.
"""
